 ...
 */
class MercurielCreateCommand extends AbstractMagentoCommand
{
    /**
     * @var array
     */
    ...
    {
        $this
            ->setName('mercuriel:module:create')
    ...
    {
        if ($this->modmanMode) {
            $modManDir = $this->vendorNamespace . '_' . $this->moduleName . '/modules';
        ...
        //directory for controllers, etc, blocks, ...
        $moduleDir = sprintf(
            '../modules/'.$this->vendorNamespace.'/'.$this->moduleName.'/app/code/%s/%s/%s',
            $this->codePool,
    ...
    {
        $pathModule = sprintf(
            '../modules/%s/%s/app/etc/modules',
            $this->vendorNamespace,
            $this->moduleName
        );
        $this->moduleDirectory = $pathModule;
        mkdir($this->moduleDirectory, 0777, true);

        $outFile = sprintf(
            '../modules/%s/%s/app/etc/modules/%s_%s.xml',
            $this->vendorNamespace,
            $this->moduleName,
            $this->vendorNamespace,
            $this->moduleName
    ...
    {
        $pathModule = sprintf(
            '../modules/%s/%s/app/code/local/%s/%s',
            $this->vendorNamespace,
            $this->moduleName,
            $this->vendorNamespace,
            $this->moduleName
